...
public class RegisterApplicationUseCase {
    ...
    public Mono<Application> register(Application application) {
        ...
        return loanTypeRepository.findById(application.getLoanType().getId())
                .switchIfEmpty(Mono.error(new BusinessException(LOAN_TYPE_DOES_NOT_EXIST.getMessage())))
                .flatMap(loanType -> {
                    BigDecimal amount = application.getAmount();
                    if (amount.compareTo(loanType.getMinimumAmount()) < 0 || amount.compareTo(loanType.getMaximumAmount()) > 0) {
                        return Mono.error(new BusinessException(AMOUNT_OUT_OF_RANGE.getMessage()));
                    }
                    return authRepository.getUserByDocument(application.getClientDocument())
                            .switchIfEmpty(Mono.error(new BusinessException(CLIENT_NOT_FOUND.getMessage())))
                            .flatMap(user -> {
                                application.setState(PENDING_REVIEW.toDomain());
